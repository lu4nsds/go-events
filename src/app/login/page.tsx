  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // If eventId is present, try to register for the event
        if (eventId) {
          try {
            ...
            if (registrationResponse.ok) {
              ...
              // If registration fails, just redirect to home
              toast.success("Login realizado com sucesso!");
              console.error("Failed to register for event after login");
            }
        ...
        <div className="space-y-8">
          <div>
            ...
            <p className="mt-2 text-center text-sm text-gray-600">
              Ou{" "}
              <Link
                href={`/register${eventId ? `?eventId=${eventId}` : ""}`}
